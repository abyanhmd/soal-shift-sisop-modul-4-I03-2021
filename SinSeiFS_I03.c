...
static const char *dirPath = "/home/hilmythoriq/Downloads";
static const char *logPath = "/home/hilmythoriq/SinSeiFS.log";

// gcc -Wall `pkg-config fuse --cflags` SinSeiFS_I03.c -o coba `pkg-config fuse --libs`
// ./coba  fusefolder
// fusermount -u fusefolder

char *key = "SISOP";
int x = 0;

void onePathLog(char *nama, char *fpath)
{
	time_t rawtime;
	struct tm *timeinfo;
	time(&rawtime);
...
void twoPathLog(char *nama, const char *from, const char *to)
{
	time_t rawtime;
	struct tm *timeinfo;
	time(&rawtime);
	...
	if (strcmp(nama, "RMDIR") == 0 || strcmp(nama, "UNLINK") == 0) {
		sprintf(str, 
				"WARNING::%.2d%.2d%d-%.2d:%.2d:%.2d::%s::%s::%s\n", 
				timeinfo->tm_mday, 
				timeinfo->tm_mon + 1, 
				timeinfo->tm_year + 1900, 
				timeinfo->tm_hour, 
...
static int xmp_getattr(const char *path, struct stat *stbuf)
{
	...
	if (atoz != NULL) decryptAtbash(atoz);
...
static int xmp_readdir(const char *path, void *buf, fuse_fill_dir_t filler, off_t offset, struct fuse_file_info *fi)
{
	...
	while ((de = readdir(dp)) != NULL){
		...
		//printf("before: %s\n", de->d_name);
		//printf("a: %s\n", a);
		if (atoz != NULL) encryptAtbash(de->d_name);
		//printf("b: %s\n", b);
		if (rx != NULL){
			//if(rot13)
...
static int xmp_rename(const char *from, const char *to) {
	...
	char *atoz = strstr(to, "AtoZ_");
	if (atoz != NULL) decryptAtbash(atoz);
	
	char *rxFrom = strstr(from, "RX_");
	if (rxFrom != NULL){
		//struct stat stats;
		//stat(from, &stats);
		//if(ctime(&stats.st_ctime) == ctime(&stats.st_mtime))
		decryptRot13(rxFrom);
		decryptAtbash(rxFrom);
		//else
		//decryptVigenere(rxFrom);
	}
	
	char *rxTo = strstr(to, "RX_");
	if (rxTo != NULL){
		//struct stat stats;
		//stat(from, &stats);
		//if(ctime(&stats.st_ctime) == ctime(&stats.st_mtime))
	...
	twoPathLog("RENAME", frompath, topath);
	
	if (rxTo != NULL){
		enkripsi2(topath);
		twoPathLog("ENCRYPT2", from, to);
	}

	if (rxFrom != NULL && rxTo == NULL){
		dekripsi2(topath);
		twoPathLog("DECRYPT2", from, to);
	}
	
	if (strstr(to, "A_is_a_") != NULL){
		encryptBinary(topath);
		twoPathLog("ENCRYPT3", from, to);
	}
	
	if (strstr(from, "A_is_a_") != NULL && strstr(to, "A_is_a_") == NULL){
		decryptBinary(topath);
		twoPathLog("DECRYPT3", from, to);
	}
...
static int xmp_symlink(const char *from, const char *to) {
	...
	twoPathLog("SYMLINK", frompath, topath);
...
static int xmp_link(const char *from, const char *to) {
	...
	twoPathLog("LINK", frompath, topath);
